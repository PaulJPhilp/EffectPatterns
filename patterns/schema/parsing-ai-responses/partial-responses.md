...
const handleStream = (chunks: AsyncIterable<string>) =>
  Effect.gen(function* () {
    ...
    for await (const chunk of chunks) {
      ...
      const attempt = yield* parseStreamed(
        accumulated
      ).pipe(
        Effect.catchTag(
          "ValidationError",
          () => Effect.none() // Not complete yet
        ),
        Effect.catchTag(
          "ParseError",
          () => Effect.none() // Still incomplete
        )
      )
    ...
    if (accumulated.length > 0) {
      const final = yield* parseStreamed(
        accumulated
      ).pipe(
        Effect.catchTag(
          "ValidationError",
          (e) => {
            console.warn(
              "Final chunk incomplete but keeping:",
              e.message
            )
