...
describe("MetricsService", () => {
  describe("incrementRequestCount", () => {
    it("should increment request count", async () => {
      ...
      await Effect.runPromise(
        program.pipe(Effect.provide(MetricsService.Default))
      );
    });
  });
...
describe("ApiError", () => {
  describe("make", () => {
    it("should create custom error", () => {
      const error = ApiError.make("Custom error", 418, "CUSTOM_ERROR", {
        extra: "data",
      });
...
describe("Error Types", () => {
  ...
  describe("RuleParseError", () => {
    it("should create rule parse error", () => {
      const error = new RuleParseError({
        file: "rule.mdc",
