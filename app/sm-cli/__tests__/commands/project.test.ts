...
describe("Project Commands", () => {
  ...
  describe("projectSet command", () => {
    it("should set active project and display success message", async () => {
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "my-project", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );

      // Check that success message was displayed
      expect(capturedOutput.some((line) => line.includes("my-project"))).toBe(
        true
      );
    ...
    it("should output JSON format when requested", async () => {
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "json-project", format: "json" }),
          Effect.provide(ConfigServiceLive)
        )
      );

      const jsonOutput = capturedOutput.join("").match(/\{[\s\S]*\}/);
  ...
  describe("projectInfo command", () => {
    ...
    it("should display API key status correctly", async () => {
      ...
      fs.writeFileSync(
        configPath,
        JSON.stringify(configWithoutKey, null, 2),
        "utf-8"
      );
