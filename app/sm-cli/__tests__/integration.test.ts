...
describe("SM-CLI Integration Tests", () => {
  ...
  describe("Full project workflow", () => {
    it("should complete a full project management workflow", async () => {
      ...
      expect(
        capturedOutput.some((line) => line.includes("effect-patterns"))
      ).toBe(true);
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "my-first-project", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );

      expect(
        capturedOutput.some((line) => line.includes("my-first-project"))
      ).toBe(true);
    ...
    it("should handle multiple project switches", async () => {
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "project-alpha", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "project-beta", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "project-gamma", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );
  ...
  describe("Error recovery", () => {
    it("should handle errors gracefully and maintain state", async () => {
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "valid-project", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );
  ...
  describe("Real-world scenarios", () => {
    it("should support DevOps workflow: multiple projects and uploads tracking", async () => {
      ...
      await Effect.runPromise(
        projectSet.pipe(
          (cmd: any) =>
            cmd.handler({ projectName: "prod-deployment", format: "human" }),
          Effect.provide(ConfigServiceLive)
        )
      );
    ...
    it("should support team collaboration: different projects for different purposes", async () => {
      ...
      for (const project of projects) {
        await Effect.runPromise(
          projectSet.pipe(
            (cmd: any) =>
              cmd.handler({ projectName: project, format: "human" }),
            Effect.provide(ConfigServiceLive)
          )
        );
    ...
    it("should survive multiple command cycles", async () => {
      ...
      for (const cmd of commands) {
        ...
        await Effect.runPromise(
          projectSet.pipe(
            (c: any) =>
              c.handler({ projectName: cmd.name, format: cmd.format }),
            Effect.provide(ConfigServiceLive)
          )
        );
      ...
      const operations = [
        () =>
          Effect.runPromise(
            projectSet.pipe(
              (cmd: any) =>
                cmd.handler({ projectName: "op1", format: "human" }),
              Effect.provide(ConfigServiceLive)
            )
          ),
        () =>
