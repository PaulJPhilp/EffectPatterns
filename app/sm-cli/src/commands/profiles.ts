...
const profilesShowCommand = Command.make(
  ...
  ({ user, section, format }) =>
    Effect.gen(function* () {
      ...
      console.log(
        chalk.gray(`Retrieved: ${formatDate(profile.retrievedAt)}\n`)
      );
      ...
      if (section === "static" || section === "both") {
        ...
        table.push([
          chalk.blue("📋 Static Profile"),
          staticFacts || chalk.gray("(empty)"),
        ]);
      }
      ...
      if (section === "dynamic" || section === "both") {
        ...
        table.push([
          chalk.yellow("🔄 Dynamic Profile"),
          dynamicFacts || chalk.gray("(empty)"),
        ]);
      }
...
const profilesSearchCommand = Command.make(
  ...
  ({ user, query, format }) =>
    Effect.gen(function* () {
      ...
      console.log(
        chalk.gray(`Retrieved: ${formatDate(result.profile.retrievedAt)}\n`)
      );
...
const profilesListCommand = Command.make(
  ...
  ({ container, limit, format }) =>
    Effect.gen(function* () {
      ...
      table.push([
        chalk.blue("Total Users"),
        chalk.green(stats.totalUsers.toString()),
      ]);
      table.push([
      ...
      table.push([
        chalk.blue("Max Static Facts"),
        chalk.cyan(stats.maxStaticFacts.toString()),
      ]);
      table.push([
...
const profilesCompareCommand = Command.make(
  "compare",
  {
    user1: Options.text("user1").pipe(Options.withDescription("First user ID")),
      ...
      if (show === "all" || show === "similarities") {
        if (comparison.commonStatic.length > 0) {
          ...
          if (comparison.commonStatic.length > 5) {
            console.log(
              chalk.gray(`  ... and ${comparison.commonStatic.length - 5} more`)
            );
          }
        }
        ...
        if (comparison.commonDynamic.length > 0) {
          ...
          if (comparison.commonDynamic.length > 5) {
            console.log(
              chalk.gray(
                `  ... and ${comparison.commonDynamic.length - 5} more`
              )
            );
          }
        }
...
export const profilesCommand = Command.make(
  ...
  Command.withSubcommands([
    ...
    profilesSearchCommand.pipe(
      Command.withDescription("Search user profile with query")
    ),
    profilesExportCommand.pipe(
      Command.withDescription("Export user profile to file")
    ),
    profilesListCommand.pipe(
      Command.withDescription("List profiles in container")
    ),
    profilesCompareCommand.pipe(
      Command.withDescription("Compare two user profiles")
    ),
    profilesStatsCommand.pipe(
      Command.withDescription("Show profile statistics")
    ),
  ])
);
