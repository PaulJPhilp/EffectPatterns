...
export const memoriesList: any = Command.make(
  ...
  (options) =>
    Effect.gen(function* () {
      ...
      memories = yield* supermemoryService.listMemories(
        options.page,
        options.limit
      );
      ...
      if (typeValue !== undefined) {
        memories = memories.filter(
          (m: any) => (m.metadata as any)?.type === typeValue
        );
      }
      ...
      if (options.format === "json") {
        ...
        const header = createHeader(
          `Memories (Page ${options.page})`,
          `Showing ${memories.length} of 640 memories`
        );
  ...
  (options) =>
    Effect.gen(function* () {
      ...
      if (options.format === "json") {
        ...
        const stats = createStatPanel(
          `Total ${typeLabel} Memories`,
          count.toString(),
          "cyan"
        );
        yield* Effect.sync(() => console.log(header + "\n" + stats));
 ...
 */
export const memoriesAdd: any = Command.make("add", {}, () =>
  Effect.gen(function* () {
    const config = yield* loadConfig;
    const supermemoryService = yield* SupermemoryServiceLive(config.apiKey);
...
export const memoriesSearch: any = Command.make(
  "search",
  {
    ...
    threshold: Options.float("threshold").pipe(
      ...
      Options.withDescription(
        "Similarity threshold (0.0-1.0): 0=broad, 1=precise"
      )
    ),
    rerank: Options.boolean("rerank").pipe(
      ...
      Options.withDescription(
        "Re-score results with different algorithm for higher accuracy"
      )
    ),
    container: Options.text("container").pipe(
      Options.optional,
      Options.withDescription(
        "Filter by container tag (user/project identifier)"
      )
    ),
    format: formatOption,
  },
  (options) =>
    Effect.gen(function* () {
      ...
      const result =
        yield* supermemoryService.searchMemoriesAdvanced(searchOptions);
      ...
      if (options.format === "json") {
        ...
        if (result.results.length === 0) {
          ...
          const message =
            createHeader(
              "Search Results",
              `Query: "${options.query}"${filterInfo}`
            ) +
            "\n" +
          ...
          const thresholdInfo = options.threshold
            ? ` | threshold: ${options.threshold}`
            : "";
          const rerankInfo = options.rerank ? " | rerank: on" : "";
          const containerInfo = options.container
...
export const documentsSearch: any = Command.make(
  "search",
  {
    ...
    rewriteQuery: Options.boolean("rewrite-query").pipe(
      ...
      Options.withDefault(false),
      Options.withDescription(
        "Expand query for better coverage (+400ms latency)"
      )
    ),
  ...
  (options) =>
    Effect.gen(function* () {
      ...
      const searchOptions: DocumentSearchOptions = {
        ...
        limit: options.limit,
        documentThreshold:
          typeof options.documentThreshold === "number"
            ? options.documentThreshold
            : 0.5,
        chunkThreshold:
          typeof options.chunkThreshold === "number"
            ? options.chunkThreshold
            : 0.6,
        rerank: typeof options.rerank === "boolean" ? options.rerank : false,
        rewriteQuery:
          typeof options.rewriteQuery === "boolean"
            ? options.rewriteQuery
            : false,
        includeFullDocs:
          typeof options.includeFullDocs === "boolean"
            ? options.includeFullDocs
            : false,
        containerTags,
      };
      ...
      if (options.format === "json") {
        ...
        if (result.results.length === 0) {
          ...
          const chunkThresholdInfo = options.chunkThreshold
            ...
            : "";
          const rerankInfo = options.rerank ? " | rerank: on" : "";
          const queryRewriteInfo = options.rewriteQuery
            ? " | query-rewrite: on"
            : "";
