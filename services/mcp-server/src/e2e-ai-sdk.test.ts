...
const searchPatternsTool = {
  description:
    "Search for Effect-TS patterns by query, category, or difficulty",
  parameters: {
    type: "object",
    properties: {
      query: {
...
const getPatternTool = {
  ...
  execute: async (args: { id: string }) => {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Get pattern failed: ${response.status} ${response.statusText}`
      );
    }
...
describe("AI SDK End-to-End Tests", () => {
  ...
  (hasOpenAIKey ? describe : describe.skip)(
    "Pattern Server AI Integration",
    () => {
      ...
      it("should be able to search patterns via API", async () => {
        const response = await fetch(
          `${TEST_SERVER_URL}/api/patterns?q=error`,
          {
            headers: {
              "x-api-key": TEST_API_KEY,
            },
      ...
      it("should be able to get pattern details via API", async () => {
        ...
        const searchResponse = await fetch(
          `${TEST_SERVER_URL}/api/patterns?q=error&limit=1`,
          {
            headers: {
              "x-api-key": TEST_API_KEY,
            },
        ...
        const detailResponse = await fetch(
          `${TEST_SERVER_URL}/api/patterns/${patternId}`,
          {
            headers: {
              "x-api-key": TEST_API_KEY,
            },
      ...
      it("should handle API errors gracefully", async () => {
        const response = await fetch(
          `${TEST_SERVER_URL}/api/patterns/non-existent-pattern`,
          {
            headers: {
              "x-api-key": TEST_API_KEY,
            },
  ...
  (hasOpenAIKey ? describe : describe.skip)("AI Tool Integration", () => {
    ...
    it("should validate pattern retrieval tool execution", async () => {
      ...
      const searchResult = await searchPatternsTool.execute({
        query: "error",
        limit: 1,
      });
      expect(searchResult.length).toBeGreaterThan(0);
