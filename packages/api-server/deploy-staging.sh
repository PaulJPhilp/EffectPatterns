...
if [ ! -d "${ROOT_DIR}/.vercel" ]; then
    ...
    echo ""
    echo "  2. Set environment variables (Vercel → Settings → Environment Variables → Preview):"
    echo "     ${GREEN}vercel env add DATABASE_URL preview${NC}   # Staging Postgres URL (use pooler for serverless)"
    echo "     ${GREEN}vercel env add PATTERN_API_KEY preview${NC}   # Staging API key (STAGING_API_KEY in tests must match)"
...
echo "  2. If staging DB was empty, seed it (use the same DATABASE_URL as in Vercel Preview):"
echo "     ${GREEN}DATABASE_URL=\"<staging-url>\" bun run ingest${NC}"
echo "  3. Run smoke tests (STAGING_API_KEY must match Preview PATTERN_API_KEY):"
