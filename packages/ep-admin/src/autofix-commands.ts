...
const validateProvider = (provider: string): Effect.Effect<AiProvider, Error> =>
	Effect.succeed(provider as AiProvider).pipe(
		Effect.filterOrFail(
			...
			() =>
				new Error(
					`Invalid provider: ${provider}. Must be: google, openai, or anthropic`,
				),
		),
	);
		...
		Effect.gen(function* () {
			...
			if (options.aiCall && !options.ai) {
				yield* Display.showWarning(
					"AI call enabled but AI prompts disabled - enabling AI prompts",
				);
			}

			if (options.write && options.dryRun) {
				yield* Display.showWarning(
					"Write mode conflicts with dry-run - dry-run will be ignored",
				);
			}

            // Determine if we should proceed with AI fixes
            if (options.aiCall || options.ai) {
                const autofix = yield* AutofixService;
                yield* autofix.fixPrepublishErrors({
                    reportPath: validatedReport,
                    provider: validatedProvider,
                    model: options.model,
                    limit: options.limit._tag === "Some" ? options.limit.value : undefined,
                    attempts: options.attempts,
                    dryRun: options.dryRun && !options.write, // write overrides dryRun
                });
            } else {
...
export const autofixCommand = Command.make("autofix").pipe(
	...
	Command.withSubcommands([autofixPrepublishCommand]),
);
