/**
 * All supported automated refactoring IDs.
 *
 * These IDs are referenced by governed rules (`RuleId`) and returned on
 * findings as `refactoringIds`. Consumers can pass these IDs into
 * `AnalysisService.applyRefactorings`.
 */
...
/**
 * String union of all supported automated refactoring IDs.
 */
export type FixId = (typeof FixIdValues)[number];

/**
 * All governed rule IDs that the analyzer can detect.
 *
 * These are the stable identifiers used across:
 * - `RuleRegistryService.listRules()`
 * - `CodeAnalyzerService` findings
 * - `AnalysisService.generateFix(...)`
 */
...
/**
 * String union of all governed rule IDs.
 */
