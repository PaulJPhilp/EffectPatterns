...
import type { FixDefinition, RuleDefinition } from "../../services/rule-registry";
...
describe("Rule Registry Integrity Tests", () => {
	...
	it("no duplicate rule IDs exist", async () => {
		...
		const ruleIds = (rules as RuleDefinition[]).map((r) => r.id);
	...
	it("every rule with fix references valid fix IDs", async () => {
		...
		const fixIds = new Set((fixes as FixDefinition[]).map((f) => f.id));
	...
	it("no duplicate fix IDs exist", async () => {
		...
		const fixIds = (fixes as FixDefinition[]).map((f) => f.id);
