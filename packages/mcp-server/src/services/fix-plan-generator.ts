/**
 * FixPlanGenerator - Generate fix plans for code findings
 *
 ...
 * @param allFixes All available fix definitions
 * @returns FixPlan with steps, changes, and risks
 */
export function generateFixPlan(
	finding: Finding,
	rule: RuleDefinition,
	allFixes: readonly FixDefinition[]
): FixPlan {
	// Get relevant fixes for this rule
	const relevantFixes = rule.fixIds
		.map((id) => allFixes.find((f) => f.id === id))
		.filter((f) => f !== undefined) as FixDefinition[];
	...
	switch (rule.category) {
		...
		case "async":
			steps.push({
				...
				detail:
					"Use Effect.all(..., { concurrency: N }) or Effect.parallelN",
			});
			steps.push({
				order: 2,
				action: "Set appropriate concurrency limit",
				detail:
					"Choose limit based on resource constraints (file handles, network connections, etc.)",
			});
			steps.push({
				order: 3,
				action: "Test under load",
				detail:
					"Verify concurrency behavior doesn't cause resource exhaustion",
			});
			break;
		...
		case "dependency-injection":
			steps.push({
				...
				detail:
					"Replace Context.Tag with modern Effect.Service pattern",
			});
			steps.push({
				order: 2,
				action: "Provide service implementation",
				detail:
					"Use Layer to provide the service in your Effect environment",
			});
			steps.push({
				order: 3,
				action: "Update consumers",
				detail:
					"Change access pattern to use Service instead of Context.Tag",
			});
			break;
		...
		case "resources":
			steps.push({
				...
				detail:
					"Determine when resources should be acquired and released",
			});
			steps.push({
				order: 2,
				action: "Use Effect.acquireRelease",
				detail:
					"Wrap resource acquisition with proper cleanup guarantees",
			});
			steps.push({
				order: 3,
	...
	switch (rule.category) {
		case "errors":
			changes.push({
				...
				scope: "This file",
				description:
					"Error type definitions or imports added",
			});
			changes.push({
				type: "modify",
		...
		case "async":
			changes.push({
				...
				description:
					"Replace async/await with Effect composition",
			});
			changes.push({
				type: "modify",
				scope: "Error handling",
				description:
					"Update from try/catch to Effect error handling",
			});
			break;
		...
		case "async":
			changes.push({
				type: "modify",
				scope: "Effect.all call",
				description:
					"Add concurrency constraint to parallel operations",
			});
			break;
 ...
 */
function generateRisks(rule: RuleDefinition): readonly string[] {
	const risks: string[] = [];
	...
	switch (rule.category) {
		...
		case "async":
			risks.push(
				"Breaking change if consumers expect Promise return type"
			);
			risks.push("May require updating all call sites");
			break;

		case "async":
			risks.push("Concurrency semantics will change - may affect timing");
			risks.push(
				"Test under realistic load to verify resource behavior"
			);
			break;
