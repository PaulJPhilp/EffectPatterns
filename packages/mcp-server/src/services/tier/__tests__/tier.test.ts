 ...
 * Tier Service Tests
 *
 * Tests for the tier validation service used by HTTP API route handlers.
 *
 * Architecture:
 * - HTTP API uses this service to validate tier access (free/paid)
 * - Returns 402 Payment Required for paid endpoints on free tier
 * - MCP server is pure transport - doesn't know about tiers
 * - All tier validation happens at HTTP API level
 */

import { Effect, Either } from "effect";
import { afterAll, beforeEach, describe, expect, it } from "vitest";
import { ConfigurationError, MCPTierService } from "../index";
...
describe("MCPTierService", () => {
	...
	const setEnv = (overrides: Record<string, string>) => {
		restoreEnv();
		// Specifically delete TIER_MODE if not in overrides to ensure "missing" test works
		if (!("TIER_MODE" in overrides)) {
