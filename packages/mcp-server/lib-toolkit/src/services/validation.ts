 ...
 */
export class ValidationServiceError extends Data.TaggedError(
    ...
    effect: Effect.gen(function* () {
      ...
      const validate = <I, A>(
        ...
        Effect.gen(function* () {
          ...
          if (isLoggingEnabled) {
            yield* logger
              .withDuration(startTime, "validate")
       ...
       */
      const createValidator = <I, A>(
        schema: Schema.Schema<I, A>,
        schemaName?: string
      ) => ({
        validate: (input: unknown) => validate(schema, input, schemaName),
        validateOrFail: (input: unknown) =>
          validateOrFail(schema, input, schemaName),
        schema,
        schemaName,
      });
