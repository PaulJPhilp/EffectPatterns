...
describe("release helpers", () => {
  it("categorizes conventional commits into appropriate buckets", async () => {
    ...
    expect(categories.features).toContain("add new user onboarding flow");
    expect(categories.fixes).toContain("correct null pointer in pipeline");
    expect(categories.docs).toContain("update README for installation");
    expect(categories.chore).toContain("bump dependencies");
    expect(categories.other).toContain("cleanup internal types");
  });
  ...
  it("generates a changelog with sections for non-empty categories", async () => {
    ...
    const categories = await categorizeCommits(commits);
    const changelog = generateChangelog(categories, "0.1.0", "0.2.0");
