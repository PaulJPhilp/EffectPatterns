...
export const getRecommendedBump = (
	_commits: string[],
): Effect.Effect<{ releaseType: 'major' | 'minor' | 'patch'; reason: string }, Error> =>
	Effect.async((resume) => {
				...
				conventionalRecommendedBump(
					...
					(err: any, result: any) => {
						if (err) {
							...
							resume(
								Effect.succeed({
									releaseType: result.releaseType as 'major' | 'minor' | 'patch',
...
export const lintInParallel = async (files: string[]): Promise<LintResult[]> => {
