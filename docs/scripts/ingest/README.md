...
flowchart TD
  ...
  subgraph RULES_QA
    D[rules-improved.ts on database\n- Loads patterns from PostgreSQL\n- Generates multiple rule formats\n- Markdown, JSON, Cursor, Windsurf formats]
    D -->|pass| E
...
- `scripts/publish/rules-improved.ts` (loads from database)
...
# 2) Rules QA
npx tsx scripts/publish/rules-improved.ts \
  --input content/raw \
  --source content/new/src
