...
async function extractGeminiText(
  ...
  const text: string | undefined = (json as any)?.candidates?.[0]?.content
    ?.parts?.[0]?.text;
  return text ?? '';
}
  ...
  try {
    ...
    try {
      ...
      const hasErrors = Array.isArray(report?.files)
        ? (report.files as Array<Record<string, unknown>>).some(
            (f) =>
              Array.isArray(f.diagnostics) &&
              (f.diagnostics as Array<Record<string, unknown>>).some(
                (d) => d.severity === 'error',
              ),
          )
        : false;
