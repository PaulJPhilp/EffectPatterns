...
`,
  ...
  const findings = report.findings.filter(
    (f) => f.ruleId === 'blocking-calls-in-effect',
  );
  ...
  if (findings.length > 0) {
    ...
    console.log(
      '❌ FAILURE: No findings found. Rule detection still failing for snippets without imports.',
    );
  ...
  .pipe(Effect.provide(AnalysisService.Default), Effect.runPromise)
  .catch((e) => {
