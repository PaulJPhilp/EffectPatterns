...
async function runAllTests() {
  ...
  try {
    ...
    await runTest("Pattern Search", async () => {
      ...
      console.log(`   Found ${searchResults.length} patterns matching "effect"`)
    })
    ...
    await runTest("Count Patterns by Skill Level", async () => {
      ...
      if (
        ...
        throw new Error("countBySkillLevel should return counts for all levels")
      }
      console.log(
    ...
    await runTest("Find Patterns by Application Pattern", async () => {
      ...
      if (aps.length > 0) {
        ...
        console.log(
          `   Found ${patterns.length} patterns for ${aps[0].name}`
        )
      } else {
