...
## 🔍 SEMANTIC VALIDATION CHECKS (NEW!)

### Semantic Check 1: Memory Behavior 💾
...
1. Does the code actually stream, or does it load everything into memory first?
   ...
   - ❌ BAD: `fs.readFileString(path)` then `Stream.fromIterable(content.split('\n'))`
   - ✅ GOOD: `fs.readFile(path).pipe(Stream.decodeText('utf-8'), Stream.splitLines)`

2. Does it use `Stream.runCollect` which loads the entire stream?

   - ⚠️ WARNING: Only acceptable if the dataset is known to be small
   - ✅ BETTER: Use `Stream.run` or process in chunks

3. Does the description claim "constant memory" or "streaming"?
   - ✅ VERIFY: The code actually uses constant memory
...
### Semantic Check 2: Concurrency Claims 🔀
...
1. Does `Effect.all` have an explicit `concurrency` option?
   ...
   - ❌ FAIL: `Effect.all(effects)` - sequential by default!
   - ✅ PASS: `Effect.all(effects, { concurrency: "unbounded" })`
   - ✅ PASS: `Effect.all(effects, { concurrency: 5 })`

2. Does the description claim "parallel" or "concurrent"?

   - ✅ VERIFY: Code actually runs in parallel
   - ❌ FAIL: Claims parallel but is sequential
...
### Semantic Check 3: Effect Idioms 🎨
...
2. Using deprecated APIs:
   ...
   - `Effect.fromOption` → Use `Option.match`
   - `Effect.fromEither` → Use `Either.match`
...
### Semantic Check 4: API Modernization 🔄
